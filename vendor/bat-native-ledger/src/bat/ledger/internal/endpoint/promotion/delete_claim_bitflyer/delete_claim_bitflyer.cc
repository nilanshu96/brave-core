...
DeleteClaimBitflyer::DeleteClaimBitflyer(LedgerImpl* ledger) : ledger_(ledger) {
  DCHECK(ledger_);
}

DeleteClaimBitflyer::~DeleteClaimBitflyer() = default;

std::string DeleteClaimBitflyer::GetUrl() {
...
type::Result DeleteClaimBitflyer::CheckStatusCode(const int status_code) {
...
void DeleteClaimBitflyer::Request(DeleteClaimBitflyerCallback callback) {
  auto url_callback =
      std::bind(&DeleteClaimBitflyer::OnRequest, this, _1, callback);
...
void DeleteClaimBitflyer::OnRequest(const type::UrlResponse& response,
